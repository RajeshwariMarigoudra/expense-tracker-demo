...
const TableData = (props) => {
  ...
  const handleEditClick = (itemKey) => {
    ...
    if (transactions.length > 0) {
      const editTran = props.data.filter((item) => item.id === itemKey); // ✅ Correct key (id)
      setCurrId(itemKey);
  ...
  const handleDeleteClick = async (itemKey) => {
    ...
    const { data } = await axios.post(`${deleteTransactions}/${itemKey}`, {
      userId: props.user.id, // ✅ Correct key (id)
    });
  ...
  const exportToPDF = () => {
    ...
    // ✅ Summary
    doc.setFontSize(12);
    // doc.text(`Total Income: ₹${totalIncome.toLocaleString()}`, 14, 25);
    // doc.text(`Total Expenses: ₹${totalExpenses.toLocaleString()}`, 14, 30);
    // doc.text(`Balance: ₹${(totalIncome - totalExpenses).toLocaleString()}`, 14, 35);
  ...
  return (
    <>
      ...
      <div className="summary-container">
        ...
        <h4>Balance: ₹{(totalIncome - totalExpenses).toLocaleString()}</h4> */}
